...
#define REMOVE_FIMC_SNAPSHOT_THREAD //jmq.define to reduce the threads count, and do sync.
#define REMOVE_FIMC_PREVIEW_THREAD   //jmq.define to reduce the threads count, and do sync.
...
private:
    ...
    public:
        ...
        mHardware(hw) { }
        virtual void onFirstRef() {
     //       run("CameraPreviewThread", PRIORITY_URGENT_DISPLAY);//lisw:cts testCancelAutoFocus(),startPreview right after camera release 
        }
        virtual bool threadLoop() {
   ...
   unsigned int         mDebugVaddr;
	...
	int mPreviewThreadWrapperRunning;
    ...
    SecCamera           *mSecCamera;
            ...
            int           takepicture_flag;//jmq.WA. for preview slide problem, now only do runSnapshotFimcOneshot after press shuttle button. TODO. find a better way
