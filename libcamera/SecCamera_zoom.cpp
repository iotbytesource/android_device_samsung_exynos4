...
#define ENHANCE_ZOOM_IN_PRECISION   //jmq.make sensor and fimc0 output as larger as possible
#define MT9D115_CROP_PATCH                 //jmq.using crop function when MT9D115. Now, we only have one image ratio of MT9D115's output,  if MT9D115 support all ratio of image, can undefine. And kernel also needs change
...
static int fimc_v4l2_s_crop(int fp,  int crop_left, int crop_top, int crop_width, int crop_height, enum v4l2_buf_type type)
{
    int ret;
    ...
    if (ioctl(fp, VIDIOC_S_CROP, &crop) < 0) {
        LOGE("%s::Erro:crop.c.left : (%d), crop.c.top : (%d), crop.c.width : (%d), crop.c.height : (%d)",
...
int SecCamera::setFimc(void)
{
...
#ifdef    ENHANCE_ZOOM_IN_PRECISION
    ...
    if (m_camera_use_ISP) {
        ...
        ret = fimc_v4l2_s_fmt(m_cam_fd, m_preview_width, m_preview_height,
                    m_preview_v4lformat, V4L2_FIELD_ANY, PREVIEW_NUM_PLANE);
        CHECK(ret);
#ifdef MT9D115_CROP_PATCH
	if(m_preview_width == 1280 && m_preview_height ==720){
           fimc_v4l2_s_crop(m_cam_fd,0, 60,640,360,V4L2_BUF_TYPE_VIDEO_CAPTURE);
       }
#endif
...
int SecCamera::startRecord(bool recordHint)
{
    ...
    if (m_camera_use_ISP) {
        ...
        CHECK(ret);
#ifdef MT9D115_CROP_PATCH
	if(m_recording_width == 1280 && m_recording_height ==720){
           fimc_v4l2_s_crop(m_rec_fd,0, 60,640,360,V4L2_BUF_TYPE_VIDEO_CAPTURE);
       }
#endif
...
int SecCamera::getExif(unsigned char *pExifDst, unsigned char *pThumbSrc, int thumbSize, bool bneedCompress)//jmq.change
{
   ...
   // if (m_camera_use_ISP) {
    if(bneedCompress){///jmq.if need compress?
        ...
        if (m_jpeg_quality >= 90)  //jmq. quality for thumnail
                                            ...
                                            int *output_size)
{
    ...
    *output_size = enc_param.size;
    ...
    m_jpeg_inbuf.num_planes = 1;
#else
if (m_camera_use_ISP){//jmq.add to support ext isp yuv sensor
    if (m_flag_record_start == 0) {
...
int SecCamera::setSceneMode(int scene_mode)
{
    ...
    if (m_params->scene_mode != scene_mode) {
        if (m_flagCreate) {
			...
			//lisw:cts testSceneMode() use mSingleFocusModeSetted to indicate the driver setted single focus 
			//m_params->focus_mode = FOCUS_MODE_SINGLE;//lisw:sync to driver single focus mode.//lisw:set single focus mode in all scene mode in driver.
        }
    }
...
//	if(focus_mode != FOCUS_MODE_TOUCH){ //lisw:this is temporary added.
//		focus_mode = FOCUS_MODE_CONTINOUS;
//	}

//    if (m_params->focus_mode != focus_mode) { //lisw:do this in SecCameraHWInterface
        if (m_flagCreate) {
			#if 0
            if (m_params->focus_mode == FOCUS_MODE_AUTO || m_params->focus_mode == FOCUS_MODE_MACRO) {
                if (fimc_v4l2_s_ctrl(m_cam_fd, V4L2_CID_CAMERA_SET_AUTO_FOCUS, AUTO_FOCUS_OFF) < 0) {
...
//			LOGE("---SecCamera new focus mode=%d,old is %d",focus_mode,m_params->focus_mode);
            if (!m_camera_use_ISP) {
                if (fimc_v4l2_s_ctrl(m_cam_fd, V4L2_CID_CAMERA_SET_AUTO_FOCUS, AUTO_FOCUS_ON) < 0) {
                        LOGE("ERR(%s):Fail on V4L2_CID_CAMERA_SET_AUTO_FOCUS", __func__);
                        return -1;
...
void SecCamera::setExifChangedAttribute()
{
    ...
    if (m_camera_use_ISP) {
        ...
        if(bv < 0)//jmq.if not support, default one
            bv = 10;
        if(ev < 0)
            ev = 10;
    }
