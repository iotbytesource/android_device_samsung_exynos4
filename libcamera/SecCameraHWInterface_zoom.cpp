...
bool CameraHardwareSec::getSupportAutoFocus(const char * sensorname)
{
    ...
    int ret = 0;
/*TODO*/
    /* modify by cym 20130307 */
#if 0
    if(!strncmp(sensorname, "S5K3H2", 10))
#else
    LOGE("****************** sensorname: %s ****************\n", sensorname);

    if((!strncmp(sensorname, "S5K4ECGX", 8))
	|| (!strncmp(sensorname, "ov5640", 6)))
#endif
    /* end modify */
        return true;
    else
...
keyvaluemap S5K4ECGX_setting[]={
        //size:preview, picture, video
	{String8(CameraParameters::KEY_SUPPORTED_PREVIEW_SIZES),String8("1280x720,640x480,320x240,176x144")},
	{String8(CameraParameters::KEY_SUPPORTED_PICTURE_SIZES),String8("2592x1936,2048x1536,1600x1200,1280x960,640x480,320x240")},
	{String8(CameraParameters::KEY_SUPPORTED_VIDEO_SIZES),String8("1920x1080,1280x720,640x480,176x144")},	
       {String8(CameraParameters::KEY_PREFERRED_PREVIEW_SIZE_FOR_VIDEO),String8("1280x720")},	
        //data format:preview, picture, video
	{String8(CameraParameters::KEY_SUPPORTED_PREVIEW_FORMATS),String8(CameraParameters::PIXEL_FORMAT_YUV420P)+String8(",")
    ...
    if ((new_focus_mode_str != NULL) && (mCameraID == SecCamera::CAMERA_ID_BACK)) {
        ...
        if (0 <= new_focus_mode) {
		    if(mSupportAutoFocusBySensor){//jmq.add
		    	if(strcmp(new_focus_mode_str, current_focus_mode_str)||(mPreviewRunning==false)||(mSingleFocusModeSetted==true)){//lisw:cts testSceneMode() use mSingleFocusModeSetted to indicate the driver setted single focus mode  //lisw:when mPreviewRunning==0(after stop preview) need reset focus mode ignoring the preview focus mode. 
...
{
    LOGV("%s JMQ:0x%x", __func__,msg_type);
